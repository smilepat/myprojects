...
| ONTOLOGY_SKILL_DB | `csat-graphdb-318` 의 **16개 마이크로스킬** + `csat-micro-skills` (Python, 3단계 lexical/syntactic/semantic) |
...
축 B — 마이크로스킬 (csat-graphdb-318 16개, 유지)
  예: 문장삽입, 빈칸추론, 함축, 어휘추론, 주제찾기, 흐름파악, ...
  → 각 스킬은 축 A의 D1~D5에 가중치 매핑 (예: 빈칸추론 ↔ D3:0.6 + D4:0.3 + D5:0.1)
...
- 출력: D1~D5 5D Radar (vocab-learn-pat 컴포넌트 재사용) + 16 마이크로스킬 추정 점수
- DoD: 동일 학습자 재진단 시 theta 변동 ±0.3 이내 (안정성)

**F2. Ontology Map (약점 시각화)**
- 시각화: 16 마이크로스킬 노드 + 의존성 엣지(`csat-graphdb-318`에서 가져옴)를 Cytoscape.js로 렌더
...
**R1 (높음): csat-graphdb-318 v4 가중치 버그가 마이크로스킬 추정을 오염**
...
**R2 (높음): Cold-start — 첫 진단 25문항만으로 16 마이크로스킬 추정이 부정확**
...
3. **5D 매핑 동결 검증** — Week 2 종료 시점에 16 microskill × 5D 매핑 테이블이 markdown 1개 파일(`docs/dimension-mapping.md`)로 존재해야 함

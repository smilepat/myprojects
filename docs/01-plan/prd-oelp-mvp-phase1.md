...
**R1 (높음 → 해소): csat-graphdb-318 v4 가중치 버그가 마이크로스킬 추정을 오염** — tracking: [csat-graphdb-318#5](https://github.com/smilepat/csat-graphdb-318/issues/5)
- 가설: dimension-mapping §2 휴리스틱이 csat-graphdb-318 keyVariables 출현 빈도와 단조 대응한다
- 검증 사이클 (2026-05-22 완료):
  - **v1**: tau median 0.400 (경계), 도메인 모순 5건 → **FAIL** ([v1 보고서](../03-analysis/synthetic-validation-c4-1-v1.md))
  - **Act**: D2_Meaning 과대 선언 4건 + D4 과대 선언 1건 → 5개 QT 행 재산정 (목적/심경/주장/요지/제목)
  - **v2**: tau median 0.600, 모순 0건 → **PASS** ([v2 보고서](../03-analysis/synthetic-validation-c4-1-v2.md)) · [dimension-mapping v2](../01-plan/dimension-mapping.md) · [oelp/lib/ontology.ts](https://github.com/smilepat/oelp/blob/main/lib/ontology.ts) 동기화 완료
- 다음 단계: Phase 2 P-1 (Recommendation v2) 진행 가능. 외부 학습자 데이터 누적 시 ridge regression으로 추가 calibration.
...
- W3~5 ✓ F1 진단 통합 코드 ([oelp c13942a](https://github.com/smilepat/oelp/commit/c13942a)) — vocab-cat-test 실제 연결 + C1.2 본인 평가 대기
- W6~8 ✓ F2 Ontology Map ([oelp 679a1ec](https://github.com/smilepat/oelp/commit/679a1ec)) — R3/C2.1 본인 think-aloud 대기
- W9~10 ✓ F3 학습 큐 + Leitner ([oelp fbd9720](https://github.com/smilepat/oelp/commit/fbd9720)) — C3.3 본인 4회 세션 대기
- W11 ✓ 합성 검증 일괄 — C1.3 PASS ([report](../03-analysis/c1-3-roundtrip.md)), C4.1 PASS after calibration ([v2](../03-analysis/synthetic-validation-c4-1-v2.md)), C4.2 FAIL on STUB_POOL scope ([report](../03-analysis/c4-2-diversity.md))
- W12 ✓ 12 C 기준 종합 평가 → [W12 평가 보고서](../04-report/phase1-w12-c-criteria-evaluation.md). **자동 6 PASS / 1 FAIL(scope) / 2 미해당, 본인 정성 5건 대기**. R1 해소, R2/R3 본인 평가 대기, R4/R6 자연 회피.
...
- [x] W11: 합성 검증 일괄 (C1.3 PASS, C4.1 PASS, C4.2 FAIL/scope) — 2026-05-22
- [x] W12: 12 C 기준 종합 평가 — [report](../04-report/phase1-w12-c-criteria-evaluation.md). 본인 정성 5건 평가 후 Phase 2 진입 결정.
